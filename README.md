# 📌UTS Pemograman Web 1 
Mata Kuliah : **Pemograman Web 1**  
Dosen Pengampu : **Nova Agustina, ST., M.Kom.**

---

## 👤 Profil Mahasiswa
- **Nama:** Fauzi Rikhshana  
- **NIM:** 23552011030  
- **Kelas:** TIF RP CNS A  

---

# 🧩 Project 1 — Website Sederhana

## 📄 Deskripsi Singkat
Project 1 merupakan pembuatan website sederhana dengan fitur:
- Halaman Utama (Informasi Perusahaan)
- Halaman Detail Informasi
- Halaman Login
- Halaman Registrasi
- Halaman Menu Utama
- Validasi Login menggunakan JavaScript (Percabangan IF)
- Footer di setiap halaman

---

## 📸 Screenshot Tampilan Website

### 🖥️ Tampilan Halaman Utama 
![alt text](https://github.com/FauziRikhsana/FauziRikhshana_23552011030_UTS_Pemograman_Web1/blob/main/Tugas%20web/menu%20utama.png?raw=true)
### 🖥️ Tampilan Halaman Login
![alt text](https://github.com/FauziRikhsana/FauziRikhshana_23552011030_UTS_Pemograman_Web1/blob/main/Tugas%20web/login.png?raw=true)
### 🖥️ Tampilan Halaman Login
![alt text](https://github.com/FauziRikhsana/FauziRikhshana_23552011030_UTS_Pemograman_Web1/blob/main/Tugas%20web/regis.png?raw=true)

> *Tambahkan screenshot lain jika diperlukan.*

---

# 🧩 Project 2 — CRUD API

## 📄 Deskripsi Singkat
Project 2 merupakan pembuatan **REST API** menggunakan:
- **Node.js**
- **Express.js**
- Data Dummy → JSON / Array

Fitur CRUD:
- **GET** (Menampilkan data)
- **POST** (Menambah data)
- **PUT** (Update data)
- **DELETE** (Hapus data)

---

## 🧪 Hasil Uji API (Bruno)
![alt text](https://github.com/FauziRikhsana/FauziRikhshana_23552011030_UTS_Pemograman_Web1/blob/main/Tugas%20web/Tes%20API%20bruno.png?raw=true)

 **GET** (Menampilkan data)
 ![alt text] (https://github.com/FauziRikhsana/FauziRikhshana_23552011030_UTS_Pemograman_Web1/blob/main/Tugas%20web/GET%20Detail.png?raw=true)
 **POST** (Menambah data)
 ![alt text](https://github.com/FauziRikhsana/FauziRikhshana_23552011030_UTS_Pemograman_Web1/blob/main/Tugas%20web/POST%20(Create).png?raw=true)
 **PUT** (Update data)
 ![alt text](https://github.com/FauziRikhsana/FauziRikhshana_23552011030_UTS_Pemograman_Web1/blob/main/Tugas%20web/PUT%20(Update).png?raw=true)
 **Delete** (Hapus data)
 ![alt text](https://github.com/FauziRikhsana/FauziRikhshana_23552011030_UTS_Pemograman_Web1/blob/main/Tugas%20web/Delete.png?raw=true)
